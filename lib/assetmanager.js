...
exports = module.exports = {
    ...
    self.hasPermission('delete', user._id, tenantId, { _id: id}, function (err, isAllowed) {
      ...
      database.getDatabase(function (error, db) {
        ...
        self.retrieveAsset({ _id: id }, function (error, assetRecs) {
          ...
          db.update('asset', { _id: id }, { _isDeleted: false }, next);
        });
      });
    });
  },
  ...
  postAsset: function (req, res, next) {
    ...
    form.parse(req, function (error, fields, files) {
      ...
      if(true) {
        self.postSharedAsset(fields, files, req, res, next);
      } else {
        self.postLocalAsset(fields, files, req, res, next);
      }
    });
  },

  postLocalAsset: function (fields, files, req, res, next) {
  },

  postSharedAsset: function (fields, files, req, res, next) {
    var self = this;
    ...
    filestorage.getStorage(repository, function (error, storage) {
      ...
      rs.on('close', function () {
        ...
        storage.processFileUpload(file, filepath, fileOptions, function (error, storedFile) {
          ...
          console.log(asset.directory, asset.filename, asset.repository);
  ...
  queryAssets: function (req, res, next) {
    ...
    if (search.hasOwnProperty('_isDeleted')) {
      search._isDeleted = search._isDeleted !== 'false';
    }
    ...
    async.each(Object.keys(search), function (key, nextKey) {
      ...
      self.retrieveAsset(query, options, function (error, assetRecs) {
        ...
        return res.status(200).json(assetRecs);
      });
    });
  },
  ...
  getAsset: function (req, res, next) {
    ...
    this.retrieveAsset({ _id: req.params.id }, options, function (error, assetRecs) {
      ...
      return res.status(200).json(assetRecs[0]);
    });
  },
  ...
  serveAsset: function (req, res, next) {
    ...
    var hasExt = req.params.id.indexOf('.') > -1;
    var assetId = hasExt ? req.params.id.split('.')[0] : req.params.id;
