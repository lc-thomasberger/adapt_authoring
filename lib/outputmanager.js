...
OutputPlugin.prototype.generateIncludesForConfig = function(config) {
  ...
  if (_.indexOf(includedPlugins, 'adapt-contrib-confidenceSlider') > -1 && _.indexOf(includedPlugins, 'adapt-contrib-slider') == -1) {
    includedPlugins.push('adapt-contrib-slider');
  }
            ...
            if (themeSettings) {
              fs.readdir(path.join(destinationFolder, Constants.Folders.Less), function(error, contents) {
                async.each(contents, function(item, eachCallback) {
                  ...
                  fs.readFile(path.join(destinationFolder, Constants.Folders.Less, item), function(err, file) {
                    if (err) {
...
OutputPlugin.prototype.writeCourseAssets = function(tenantId, courseId, destinationFolder, jsonObject, next) {
  ...
  async.waterfall([
    ...
    function getCourseAssets(pDb, cb) {
      db = pDb;
      db.retrieve('courseasset', { _courseId: courseId }, function(error, results) {
        courseassets = results;
        cb(error, _.pluck(results, '_assetId'));
      });
    },
    // retrieve every asset in the course, or return early if none
    function getAssets(results, cb) {
      if (results) {
        origin().assetmanager.retrieveAsset({ _id: {$in: results} }, cb);
      } else {
        ...
        filestorage.getStorage(asset.repository, function (error, storage) {
          ...
          storage && storage.createReadStream(asset.path, function(ars) {
            ars.on('error', function(error) {
              logger.log('error', 'Error copying ' + asset.title + ' (' + asset.path + ')' + ' to ' + outputFilename + ": " + error.message);
              var instances = _.where(courseassets, { _assetId: asset._id.toString() });
              async.each(instances, function(ca, cb2) {
                var map = {
                  'page': 'contentObject',
                  'menu': 'contentObject'
                };
                db.retrieve(map[ca._contentType] || ca._contentType, { _id: ca._contentTypeId }, function(error, content) {
                  if(error) {
                  ...
                  if(!content.length) {
                    logger.log('error', (map[ca._contentType] || ca._contentType) + ' with _id ' + ca._contentTypeId + ' not found');
                    cb2();
                  }
                  content = content[0];
                  var typeName = content['_' + content._type];
                  logger.log('error', `Unknown asset '${asset.title}' used in ${typeName || content._type} ${content.displayTitle || content.title} (${content._id})`);
                  cb2();
                });
              }, callback);
            });
            ars.on('end', callback);
            ars.pipe(fs.createWriteStream(outputFilename));
          });
        });
      }, cb);
    }
