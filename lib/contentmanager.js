...
ContentManager.prototype.setupRoutes = function () {
  ...
  rest.get('/content/schema', function(req, res, next) {
    var schemas = helpers.cloneObject(self._contentSchemas);
    ...
    database.getDatabase(function(err, db) {
      ...
      async.eachSeries(plugins, function(plugin, outerCallback) {
            ...
            function(callback) {
              async.eachSeries(results, function(schema, processCallback) {
                ...
                */
                ...
                /*
                * 2. Store pluginLocations
                */

                switch (plugin.type) {
                  case 'component':
                    var baseAttributes = _.clone(self._contentSchemas['component'].properties);
                  ...
                  case 'menu':
                  case 'theme':
                    var schemaPluginLocations = schema.properties && schema.properties.pluginLocations && schema.properties.pluginLocations.properties;
                    ...
                    async.each(Object.keys(schemaPluginLocations), function(key, nestedCallback) {
                      var pluginLocations = schemaPluginLocations[key];
                      if (!pluginLocations.properties) {
                        return nestedCallback();
                      }

                      // NOTE assume single top-level object
                      ...
                      switch(plugin.type) {
                        case 'extension': typeObj = contentModelExtensions; break;
                        case 'menu':      typeObj = contentModelMenus;      break;
                        case 'theme':     typeObj = contentModelThemes;     break;
                      }

                      var commonKeys = _.intersection(Object.keys(typeObj[key]), Object.keys(pluginLocations.properties));
                      if(commonKeys.length > 0) {
                        logger.log('warn', 'Already found ' + plugin.type + ' ' + key + ' attribute with key ' + keys[0] + ' (' + schema.name + ')');
                        return nestedCallback();
                      }
                      typeObj[key] = _.extend(typeObj[key], pluginLocations.properties);

                      // HACK needed to check against applied menu/theme in schemas.js
        ...
        async.each(Object.keys(schemas), function(key, callback) {
          ...
          // themes
          if(!_.isEmpty(contentModelThemes[key])) {
            if(!value.properties.themeSettings.properties) {
              value.properties.themeSettings.properties = contentModelThemes[key];
            } else {
              value.properties.themeSettings.properties = _.extend(value.properties.themeSettings.properties, contentModelThemes[key]);
            }
          }
  ...
  rest.post('/content/:type', function (req, res, next) {
    ...
    self.create(type, val, function (error, result) {
      if (error) {
  ...
  rest.get('/content/:type', function (req, res, next) {
    ...
    self.retrieve(type, search, options, function (error, results) {
      if (error) {
        res.statusCode = error instanceof ContentTypeError ? 400 : 500;
        return res.json({success: false, message: error.message});
      }
  ...
  rest.get('/content/:type/query', function (req, res, next) {
    ...
    async.each(
      ...
      self.retrieve(type, query, options, function (error, results) {
        if (error) {
          res.statusCode = error instanceof ContentTypeError ? 400 : 500;
          return res.json({ success: false, message: error.message });
        }
  ...
  rest.get('/content/:type/:id', function (req, res, next) {
    ...
    self.retrieve(type, { _id: id }, options, function (error, results) {
      if (error) {
        res.statusCode = error instanceof ContentTypeError ? 400 : 500;
        return res.json({ success: false, message: error.message });
      }
  ...
  rest.put('/content/:type/:id', function (req, res, next) {
    ...
    self.update(type, { _id: id }, val, function (error, updatedDocument) {
      if (error) {
        res.statusCode = error instanceof ContentTypeError ? 400 : 500;
        return res.json({ success: false, message: error.message });
      }
  ...
  rest.put('/content/:type/switch/:id/:switchid', function (req, res, next) {
    ...
    self.update(type, { _id: id }, val, function (error, updatedDocument) {
      ...
      self.update(type, { _id: switchId }, switchVal, function (error, updatedDocument) {
        if (error) {
  ...
  rest.delete('/content/:type/:id', function (req, res, next) {
    ...
    self.destroy(type, { _id: id }, function (error) {
      if (error) {
        res.statusCode = error instanceof ContentTypeError ? 400 : 500;
        return res.json({success: false, message: error.message});
      }
              ...
              function(callback) {
                ...
                if (clipboard['contentObjects'].length !== 0) {
                  async.eachSeries(clipboard['contentObjects'], function(item, cb) {
                    ...
                    self.create('contentobject', item, function (error, newItem) {
                      if (error) {
                        cb(error);
                      }
              ...
              function(callback) {
                ...
                if (clipboard['articles'].length !== 0) {
                  async.eachSeries(clipboard['articles'], function(item, cb) {
                    ...
                    self.create('article', item, function (error, newItem) {
                      if (error) {
                        cb(error);
                      }
              ...
              function(callback) {
                ...
                if (clipboard['blocks'].length !== 0) {
                  async.eachSeries(clipboard['blocks'], function(item, cb) {
                    ...
                    self.create('block', item, function (error, newItem) {
                      if (error) {
                        cb(error);
                      }
              ...
              function(callback) {
                ...
                if (clipboard['components'].length !== 0) {
                  async.eachSeries(clipboard['components'], function(item, cb) {
                    ...
                    self.create('component', item, function (error, newItem) {
                      if (error) {
                        cb(error);
                      }
 ...
 * @param {object} db - the database self was created
 * @param {callback} next
 */
    ...
    return {
        ...
        moduleLoaded : function(modloaded){
          var preloader = this;
           ...
           //is the module self loaded this modules requirement
